...
{
    public sealed class BondSecurityFactory : SecurityFactory
    {
        public const string Columns = "SECID,BOARDID,SHORTNAME,CURRENCYID,DECIMALS,FACEVALUE,LOTSIZE,PREVADMITTEDQUOTE";

        private BondSecurityFactory() { }

        public static BondSecurityFactory Instance { get; } = new();

        private static bool TryCreate(JsonElement securityElement, [NotNullWhen(true)] out BondSecurity? security)
        {
            security = default;
        ...
        {
            bool result = TryCreate(securityElement, out BondSecurity? bondSecurity);
            security = bondSecurity;
            return result;
        }
    }
}
